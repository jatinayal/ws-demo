...
export const seedData = async (payload: Payload, options: SeedOptions = {}) => {
  ...
  payload.logger.info(
    `Starting deterministic seed process (env: ${environment}, reset: ${reset})...`,
  );
  ...
  const existingUsers = await payload.find({
    collection: 'users',
    where: { email: { equals: 'admin@womenssynergy.org' } },
    limit: 1,
  });
  if (existingUsers.totalDocs === 0) {
    ...
    payload.logger.info(`Admin user created with ID: ${admin.id}`);
  } else {
    payload.logger.info(`Admin user exists with ID: ${existingUsers.docs[0].id}`);
  }
  ...
  const existingMedia = await payload.find({
    collection: 'media',
    where: { alt: { equals: 'Dummy Cover Image' } },
    limit: 1,
  });
  ...
  if (!nav.mainMenu || nav.mainMenu.length === 0 || reset) {
    await payload.updateGlobal({
      ...
      data: {
        announcementBar: {
          enabled: true,
          text: 'Join us for our Annual Gala 2026!',
          link: '/events',
        },
        mainMenu: [
          { label: 'Home', url: '/' },
    ...
    await payload.updateGlobal({
      ...
      data: {
        hero: {
          ...
          subheading:
            'Join us in our mission to provide education, resources, and mentorship to women around the globe.',
          callsToAction: [
        ...
        programsSection: {
          ...
          description: 'Discover how we are making a difference.',
        },
        impactSection: {
          heading: 'Our Global Impact',
          description: 'Numbers that represent lives changed.',
        },
        storiesSection: {
          heading: 'Success Stories',
          description: 'Real stories from the women whose lives have been transformed.',
        },
        eventsSection: {
        ...
        partnersSection: {
          ...
          description: 'We are incredibly grateful for the support.',
        },
        donationCta: {
          heading: 'Make a Difference Today',
          description: 'Your contribution helps us expand our reach.',
  ...
  const seedProgram = async (slug: string, data: Record<string, unknown>) => {
  ...
  await seedProgram('tech-entrepreneurship-incubator', {
  ...
  const seedEvent = async (slug: string, data: Record<string, unknown>) => {
    const existing = await payload.find({
      collection: 'events',
      where: { slug: { equals: slug } },
    });
  ...
  await seedEvent('women-leadership-summit-2026', {
    ...
    relatedPrograms: [p1 as never],
    registrationSettings: { registrationOpen: true, capacity: 500 },
  });

  const seedStory = async (slug: string, data: Record<string, unknown>) => {
    const existing = await payload.find({
  ...
  await seedStory('aisha-rahman-leadership-journey', {
    personName: 'Aisha Rahman',
    slug: 'aisha-rahman-leadership-journey',
    storyStatus: 'published',
    program: p1 as never,
    image: mediaId as never,
  ...
  // Impact Statistics
  const seedStat = async (label: string, data: Record<string, unknown>) => {
    const existing = await payload.find({
  ...
  await seedStat('Women Empowered', {
  ...
  // Partners
  const seedPartner = async (name: string, data: Record<string, unknown>) => {
    const existing = await payload.find({
      collection: 'partners',
  ...
  await seedPartner('Global Empowerment Fund', {
    name: 'Global Empowerment Fund',
    logo: mediaId as never,
    type: 'ngo',
  });
  await seedPartner('Tech for Good Foundation', {
  ...
  if (environment !== 'production' || reset) {
    const seedCRM = async (
      ...
      const existing = await payload.find({
        ...
        where: { [queryField]: { equals: value } },
      });
      if (existing.totalDocs === 0) await payload.create({ collection: collection as never, data });
    };
    ...
    await seedCRM('volunteers', 'email', 'elena.martinez@example.com', {
      ...
      phone: '+1 555-0198',
      areasOfInterest: ['mentorship', 'events'],
      message: 'Experience in corporate training.',
      status: 'pending',
    });
    ...
    await seedCRM('donation-leads', 'email', 'm.chen@example.com', {
      ...
      amount: 5000,
      frequency: 'one_time',
      message: 'Tech Incubator donation.',
      status: 'contacted',
    });
    ...
    await seedCRM('partnership-requests', 'email', 'david.wright@innovatecorp.com', {
      ...
      email: 'david.wright@innovatecorp.com',
      partnershipType: 'corporate',
      proposal: 'Hackathon sponsorship.',
      status: 'new',
    });
  }
